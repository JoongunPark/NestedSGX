...
sgx_status_t SGX_CDECL dmalloc_test(void** retval, size_t i)
{
	...
	ms_dmalloc_test_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_dmalloc_test_t);
	void *__tmp = NULL;
	...
	ms = (ms_dmalloc_test_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_dmalloc_test_t));

	ms->ms_i = i;
	status = sgx_ocall(1+10000, ms);
...
sgx_status_t SGX_CDECL dmalloc(void** retval, size_t i)
{
	...
	ms_dmalloc_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_dmalloc_t);
	void *__tmp = NULL;
	...
	ms = (ms_dmalloc_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_dmalloc_t));

	ms->ms_i = i;
	status = sgx_ocall(2+10000, ms);
...
sgx_status_t SGX_CDECL dfree(void* i)
{
	...
	ms_dfree_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_dfree_t);
	void *__tmp = NULL;
	...
	ms = (ms_dfree_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_dfree_t));

	ms->ms_i = SGX_CAST(void*, i);
	status = sgx_ocall(3+10000, ms);
