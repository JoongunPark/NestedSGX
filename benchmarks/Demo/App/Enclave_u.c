...
sgx_status_t ocall_pointer_attr(sgx_enclave_id_t eid)
{
	...
	ocall_table.nr_ocall = 10000 + ocall_table_DEnclave.nr_ocall;
	sgx_status_t status;
	status = sgx_ecall(eid, 20, &ocall_table, NULL);
	return status;
}
...
sgx_status_t ecall_array_in(sgx_enclave_id_t eid, int arr[4])
{
	...
	ms.ms_arr = (int*)arr;
	status = sgx_ecall(eid, 22, &ocall_table, &ms);
	return status;
}
...
sgx_status_t ecall_function_calling_convs(sgx_enclave_id_t eid)
{
	...
	ocall_table.nr_ocall = 10000 + ocall_table_DEnclave.nr_ocall;
	sgx_status_t status;
	status = sgx_ecall(eid, 26, &ocall_table, NULL);
	return status;
}
...
sgx_status_t ecall_function_public(sgx_enclave_id_t eid)
{
	...
	ocall_table.nr_ocall = 10000 + ocall_table_DEnclave.nr_ocall;
	sgx_status_t status;
	status = sgx_ecall(eid, 27, &ocall_table, NULL);
	return status;
}
...
sgx_status_t ecall_function_private(sgx_enclave_id_t eid, int* retval)
{
	...
	status = sgx_ecall(eid, 28, &ocall_table, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t ecall_map(sgx_enclave_id_t eid)
{
	...
	ocall_table.nr_ocall = 10000 + ocall_table_DEnclave.nr_ocall;
	sgx_status_t status;
	status = sgx_ecall(eid, 32, &ocall_table, NULL);
	return status;
}
...
sgx_status_t ecall_increase_counter(sgx_enclave_id_t eid, size_t* retval)
{
	...
	status = sgx_ecall(eid, 33, &ocall_table, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t ecall_consumer(sgx_enclave_id_t eid)
{
	...
	ocall_table.nr_ocall = 10000 + ocall_table_DEnclave.nr_ocall;
	sgx_status_t status;
	status = sgx_ecall(eid, 35, &ocall_table, NULL);
	return status;
}
