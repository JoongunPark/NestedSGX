...
//	printk("hahahah %s is called", __func__);
	...
	//vmcs_write64(ENCLS_EXITING_BITMAP, (1<<15));
//	secondary_exec_ctl |= (1<<16);
...
static void vmx_update_encls_vmexit(struct kvm_vcpu *vcpu)
{
	...
	vmx_set_encls_vmexit_bitmap(ENCLS_EXITING_BITMAP, (1 << 16) | (1 << 17) | (1 << 18) | (1 << 19));
	//printk("1");
//	vmx_enable_encls_vmexit_all(vcpu);
}
...
//		vmx_sgx_lepubkeyhash_load(vcpu);
	...
	/* custom secondary_exec_control */
	if (guest_cpuid_has_sgx_launch_control(vcpu) &&
			cpu_sgx_lepubkeyhash_writable()){
		vmx_sgx_lepubkeyhash_load(vcpu);
		vmx_set_encls_vmexit_bitmap(ENCLS_EXITING_BITMAP, (1 << 16));
		printk("%s is called in kvm", __func__);
	}
}
...
//		u64 encls_exiting_bitmap = 0;

		vmcs_write32(SECONDARY_VM_EXEC_CONTROL,
				vmx_secondary_exec_control(vmx));
//		encls_exiting_bitmap |= (1 << 16);
//		vmcs_write64(ENCLS_EXITING_BITMAP, encls_exiting_bitmap);
	}
...
{
	unsigned long rax = kvm_register_readl(vcpu, VCPU_REGS_RAX);

	printk("%s: handling encls exit reason form vcpu%d\n", __func__, vcpu->vcpu_id);
	...
	if(rax == EABC){
		printk("EBAC");
		kvm_register_writel(vcpu, VCPU_REGS_RAX, 0);
		//kvm_inject_gp(vcpu, 0);
		return 1;
		//kvm_inject_gp(vcpu, 0);
	}
	else if(rax == OECREATE){
		printk("OECREATE");
		kvm_register_writel(vcpu, VCPU_REGS_RAX, 10);
	}
	else if(handle_unexpected_encls(vcpu)){
		return 1;
	}
